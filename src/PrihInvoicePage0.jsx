...
export default function PrihInvoicePage({
  ...
  const isMoveInvoice = Number(header?.IdSklPer || 0) !== 0;
...
useEffect(() => {
  ...
  async function loadAll() {
    ...
    try {
      const [invoiceResponse, sklResponse, formResponse, rawResponse] =
        await Promise.all([
          fetchWithAuth(
...
setPostList(Array.isArray(postData) ? postData : []);
  ...
  try {
const [postResponse, formResponse, rawResponse] = await Promise.all([
...
${deletedXml}
 ...
 async function handleSave() {
  ...
  if (loading) {
    return <p>Загрузка приходной накладной...</p>;
  }
...
<SearchableSelect
  ...
  onChange={(value) => updateRow(row.ID, { Tov: value })}
/>
                </td>
