...
export default function SchetViewPage({
  ...
  if (loading) return <div className="schet-view-page schet-editor-page">Загрузка счета...</div>;

  if (error) {
    return (
      <div className="schet-view-page schet-editor-page">
        <button type="button" className="back-to-list-button prih-back-button schet-back-button" onClick={handleBackClick}>← К списку заказов</button>
    ...
    <div className="schet-view-page schet-editor-page">
      ...
      <section className="schet-body-section schet-body-panel">
        <div className="schet-body-title schet-styled-title">
          <strong>Проданные блюда</strong>
          <span>Сумма: {formatNumber(bodyTotals.summ)}</span>
          <span>
            Сумма с учетом скидки: {formatNumber(bodyTotals.summSkid)}
          </span>
          <span>Сумма себестоимости: {formatNumber(bodyTotals.seb)}</span>
        </div>
